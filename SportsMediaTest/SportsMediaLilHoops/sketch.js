...
function setup() {
  ...
  if (isMobile) {
    buttonImg.size(370 * scale, 150 * scale);
    let canvas = document.querySelector('canvas');
    // Position button in scaled coordinates
    buttonImg.position(
      canvas.offsetLeft + (width/2 - 370/2) * scale,
      canvas.offsetTop + (height/2 + 100) * scale
    );
  } else {
...
function windowResized() {
  if (isMobile) {
    // Only scale once on load for mobile, do nothing on resize
  } else {
    let canvas = document.querySelector('canvas');
    buttonImg.position(canvas.offsetLeft + width/2 - buttonImg.width/2, canvas.offsetTop + height/2 + 100);
  }
